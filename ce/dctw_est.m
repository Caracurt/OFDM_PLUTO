function [H_sw, Info] = dctw_est(H_noisy, GenPar, ChanInfo)
    ...
    A_dft_int = dct(eye(Nfft*GenPar.comb)) ./ sqrt(GenPar.comb);
    
    % init determine window sizes
    win_max = fix( 2 * ChanInfo.tau_max * GenPar.delta_f * Nfft * GenPar.comb  );
    win_min = fix( 2 * ChanInfo.win_guard * Nfft * GenPar.comb / 512);
